        ...
        if not Path.exists(full_path) or overwrite:
            ...
            if sub_df.SMA_set.iloc[-1] == "cpoke":
                if not sub_df.stimuli_on.iloc[-1]:
                    ...
                    multiplot_cpoke_pre_rule(
                        sub_df, mode=mode, save_out=save_out, save_path=full_path
                    )
                else:
                    print("stage specific plot not made yet, reverting to last plot")
                    multiplot_cpoke_pre_rule(
                        sub_df, mode=mode, save_out=save_out, save_path=full_path
                    )
...
def multiplot_cpoke_pre_rule(
    ...
    mode : str (default="settling_in")
        whether to plot cpoke distrubtions for settling in or violations
